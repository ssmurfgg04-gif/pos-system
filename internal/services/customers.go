...
func (s *Service) SettleTab(orderID int64, method, receiptCode string, p *auth.Principal) (*models.Order, error) {
        ...
        // Ledger payment posted after the guarded transition. Loyalty
        // earning happens inside completePayment now (all paid orders),
        // so this path only records the tab repayment.
        tx, err := s.db.Begin()
        if err != nil {
                return result, err
        }
