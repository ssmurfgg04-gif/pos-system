...
// 127.0.0.1 only, and shows the UI in a native OS window (Windows WebView2;
// default browser elsewhere) once the server is up. Closing the window
// quits the server. Running with no arguments always means desktop mode —
// "download, double-click, sell".
...
func runDesktop() int {
	...
	// for it. The window call blocks; returning afterwards exits without
	...
	meta := &desktopMeta{
		Port:     port,
		FirstRun: firstRun,
		// The window blocks until closed; closing it quits the server, so
		// the app's lifecycle is the window's lifecycle — like any desktop
		// app. (Fallback path keeps the server up for its browser tab.)
		OnReady: func() {
			if openAppWindow("http://127.0.0.1:"+port, dir) {
				close(quit)
			}
		},
	}
